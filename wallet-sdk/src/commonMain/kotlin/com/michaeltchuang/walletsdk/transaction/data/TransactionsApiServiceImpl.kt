...
class TransactionsApiServiceImpl(
    ...
    override suspend fun getTransactionParams(): Result<TransactionParams> = try {
    ...
    override suspend fun postTrackTransaction(
        trackTransactionRequest: TrackTransactionRequest
    ): Result<Unit> = try {
        val response: HttpResponse = httpClient.post("${getNodeBaseUrl()}/v1/transactions/") {
    ...
    override suspend fun sendSignedTransaction(
        rawTransactionData: ByteArray
    ): Result<SendTransactionResponse> = try {
        val response: HttpResponse = httpClient.post("${getNodeBaseUrl()}/v2/transactions") {
